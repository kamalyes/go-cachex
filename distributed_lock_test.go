...
// 原 bug：Unlock 返回 ErrLockNotFound 但 acquired 仍为 true，下次 TryLock 误判成功
func TestDistributedLock_UnlockNotFoundClearsState(t *testing.T) {
	client := setupRedisClient(t)
	defer client.Close()
	...
	// lockA 尝试 Unlock，应返回 ErrLockNotFound 并清理本地状态
