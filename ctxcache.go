...
func (c *CtxCache) Get(ctx context.Context, key []byte) ([]byte, error) {
	...
	case <-ctx.Done():
		return nil, ctx.Err()
	default:
		return c.handler.GetWithCtx(ctx, key)
	}
}
...
func (c *CtxCache) Set(ctx context.Context, key, value []byte) error {
	...
	case <-ctx.Done():
		return ctx.Err()
	default:
		return c.handler.SetWithCtx(ctx, key, value)
	}
}
...
func (c *CtxCache) SetWithTTL(ctx context.Context, key, value []byte, ttl time.Duration) error {
	...
	case <-ctx.Done():
		return ctx.Err()
	default:
		return c.handler.SetWithTTLAndCtx(ctx, key, value, ttl)
	}
}
...
func (c *CtxCache) Del(ctx context.Context, key []byte) error {
	...
	case <-ctx.Done():
		return ctx.Err()
	default:
		return c.handler.DelWithCtx(ctx, key)
	}
}
...
func (c *CtxCache) GetOrCompute(ctx context.Context, key []byte, ttl time.Duration, loader func(context.Context) ([]byte, error)) ([]byte, error) {
	...
	// 先尝试命中
	if val, err := c.handler.GetWithCtx(ctx, key); err == nil {
		return val, nil
	}
	...
	if data, ok := v.([]byte); ok {
		if data != nil { // 只有非 nil 数据才写入缓存
			if ttl <= 0 {
				_ = c.handler.SetWithCtx(ctx, key, data)
			} else {
				_ = c.handler.SetWithTTLAndCtx(ctx, key, data, ttl)
			}
		}
		return data, nil
	}
...
// BatchGet 批量获取多个键的值
func (c *CtxCache) BatchGet(ctx context.Context, keys [][]byte) ([][]byte, []error) {
	if c.handler == nil {
	...
	return c.handler.BatchGetWithCtx(ctx, keys)
}
...
func (c *CtxCache) GetTTL(ctx context.Context, key []byte) (time.Duration, error) {
	return c.handler.GetTTLWithCtx(ctx, key)
}
